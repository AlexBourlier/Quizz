...
function MessageBubble({ message, currentUserId, isBlocked, onReport, onUserClick, }) {
    ...
    return (_jsxs("article", { className: `group relative rounded-xl border p-3 ${isBot ? "border-amber-500/30 bg-amber-950/30" : "border-white/5 bg-ink/70"}`, onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), children: [_jsxs("div", { className: "mb-1 flex items-center justify-between", children: [_jsxs("strong", { className: `text-sm ${!isSelf && !isBot ? "cursor-pointer hover:underline" : ""}`, style: color ? { color } : undefined, onClick: !isSelf && !isBot ? () => onUserClick({
                            userId: message.user.id,
                            username: message.user.username,
                            userRole: message.user.role ?? "user",
                            messageContent: message.content,
                            messageAt: message.createdAt,
                        }) : undefined, children: [isBot && _jsx("span", { className: "mr-1 text-xs font-normal text-amber-400/70", children: "[BOT]" }), message.user.username, isSelf && _jsx("span", { className: "ml-1 text-xs font-normal text-slate-500", children: "(vous)" }), isBlocked && revealed && (_jsx("span", { className: "ml-1 text-[10px] font-normal text-slate-600", children: "(bloqu\u00E9)" }))] }), _jsxs("div", { className: "flex items-center gap-2", children: [hovered && !isSelf && !isBot && message.user.role !== "admin" && (_jsx("button", { type: "button", onClick: () => onReport({
                                    userId: message.user.id,
                                    username: message.user.username,
...
export function ChatPanel({ roomId, messages, typingUsers, currentUserId, onQuizClick, onDmUser }) {
