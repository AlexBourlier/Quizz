...
export function ProfileModal({ onClose }) {
    ...
    const handleUnblock = (blockedId, username) => {
        const socket = getSocket();
        if (!socket)
            return;
        socket.emit("contact:unblock", { userId: blockedId }, (res) => {
            if (res.ok) {
