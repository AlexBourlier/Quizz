...
export async function createReport(data) {
    ...
    sendAdminAlert(`Nouveau signalement — ${reported?.username ?? "inconnu"}`, `<p><strong>${reporter?.username ?? "??"}</strong> a signalé <strong>${reported?.username ?? "??"}</strong>.</p>
     <p>Contexte : ${data.context}</p>
     ${data.reason ? `<p>Raison : ${data.reason}</p>` : ""}
     ${data.messageContent ? `<p>Message : <em>${data.messageContent}</em></p>` : ""}
     <p><a href="${env.FRONTEND_URL}">Accéder à l'application</a></p>`).catch(() => undefined);
    return report;
}
