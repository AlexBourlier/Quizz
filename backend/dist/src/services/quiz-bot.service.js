...
import { applyScore, createOrActivateGame, endGame, getLeaderboard, getRandomQuestion, saveRoundHistory } from "./quiz.service.js";
const rounds = new Map();
const answerCooldown = new Map();
let cachedBotUserId = undefined;
...
async function postBotMessage(io, roomId, content) {
...
function cooldownKey(roomId, userId) {
    return `${roomId}:${userId}`;
}
function closeThreshold(value) {
    if (value.length <= 5)
        return 1;
    if (value.length <= 9)
        return 2;
...
export class QuizBotEngine {
    ...
    async advanceHint(io, roomId) {
        const state = rounds.get(roomId);
        if (!state)
            return { ok: false, reason: "Aucun quiz actif dans ce salon" };
        if (state.hintTimeouts.length === 0)
            return { ok: false, reason: "Plus d'indices disponibles pour cette question" };
    ...
    stop(roomId) {
        ...
        for (const t of state.hintTimeouts)
            clearTimeout(t);
        rounds.delete(roomId);
    }
    async submitAnswer(io, socket, payload) {
        ...
        const board = leaderboard
            .map((e, i) => `${i + 1}. ${e.user.username} — ${e.score} pts`)
            .join("\n");
        await postBotMessage(io, payload.roomId, `Bravo ${payload.username} ! La reponse etait : "${state.answer}" (+${points} pts)\n\nClassement :\n${board}`);
    ...
    async nextQuestion(io, roomId, gameId, filters) {
        ...
        await postBotMessage(io, roomId, `[${question.category}] ${question.question}\nIndice : ${initialHint}`);
        ...
        for (let i = 1; i <= 3; i++) {
            const t = setTimeout(() => {
                void (async () => {
                    ...
                    active.hintsUsed += 1;
                    const hint = buildHintFromPositions(active.answer, new Set(active.shuffledIndices.slice(0, active.revealedCount)));
