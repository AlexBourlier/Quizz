...
import { logSecurity, countRecentRegistrationsByIp, } from "./security-log.service.js";
...
export async function loginAsGuest() {
    ...
    if (!username)
        username = `Invité${Date.now() % 100000}`;
...
export async function handleParentalConsent(token, action) {
    const user = await prisma.user.findFirst({ where: { parentalConsentToken: token } });
    if (!user)
        throw new Error("Lien de consentement invalide ou déjà utilisé");
    if (action === "approve") {
        await prisma.user.update({
            where: { id: user.id },
