...
async function deleteExpiredDms() {
    ...
    const pendingReportedIds = await prisma.reportedMessage
        ...
        .then((rows) => rows.map((r) => r.originalMessageId));
    ...
    if (dmCount > 0)
        console.log(`[cleanup] ${dmCount} messages privés expirés supprimés`);
}
async function deleteExpiredReportedMessages() {
    ...
    if (count > 0)
        console.log(`[cleanup] ${count} signalements DM expirés supprimés`);
}
async function deleteExpiredAuditLogs() {
    ...
    if (count > 0)
        console.log(`[cleanup] ${count} audit logs expirés supprimés`);
}
async function deleteExpiredGuests() {
    ...
    if (count > 0)
        console.log(`[cleanup] ${count} comptes invités expirés supprimés`);
}
