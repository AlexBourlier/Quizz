...
function dmExpiresAt(): Date {
  const d = new Date();
  d.setDate(d.getDate() + env.DM_RETENTION_DAYS);
  return d;
}

function reportedMsgExpiresAt(): Date {
  const d = new Date();
...
export async function sendDm(senderId: string, recipientUsername: string, plainContent: string) {
  const recipient = await prisma.user.findUnique({
    where: { username: recipientUsername },
    select: { id: true, username: true },
  });
  if (!recipient) throw new Error(`Utilisateur "${recipientUsername}" introuvable`);
  if (recipient.id === senderId) throw new Error("Impossible de s'envoyer un message à soi-même");

  if (await isBlockedBy(recipient.id, senderId)) throw new Error("Vous ne pouvez pas contacter cet utilisateur");
  if (await isBlockedBy(senderId, recipient.id)) throw new Error("Vous avez bloqué cet utilisateur");
  if (!(await areContacts(senderId, recipient.id))) throw new Error("Vous devez être contacts pour envoyer un message privé");
...
export async function getDmHistory(userAId: string, userBId: string, limit = 50) {
...
export async function getDmContacts(userId: string) {
...
export async function markDmsRead(recipientId: string, senderId: string) {
...
export async function deleteConversation(userAId: string, userBId: string) {
