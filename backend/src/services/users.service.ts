...
export async function setPendingAvatar(userId: string, base64: string) {
  return prisma.user.update({
    where: { id: userId },
...
export async function approveAvatar(userId: string) {
  const user = await prisma.user.findUniqueOrThrow({
    where: { id: userId },
    select: { pendingAvatar: true }
  });
  if (!user.pendingAvatar) throw new Error("Aucun avatar en attente");
  return prisma.user.update({
    where: { id: userId },
...
export async function rejectAvatar(userId: string) {
  return prisma.user.update({
    where: { id: userId },
