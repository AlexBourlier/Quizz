...
roomsRouter.patch("/:roomId", requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
...
roomsRouter.delete("/:roomId", requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
...
roomsRouter.post("/:roomId/invite", requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
...
roomsRouter.get("/:roomId/moderators", requireRole(["admin"]), async (req: Request, res: Response) => {
  const mods = await getRoomModerators(req.params.roomId);
  return res.json(mods);
});

roomsRouter.post("/:roomId/moderators", requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
    const { username } = z.object({ username: z.string() }).parse(req.body);
    const target = await prisma.user.findUnique({ where: { username }, select: { id: true } });
    if (!target) return res.status(404).json({ message: "Utilisateur introuvable" });
...
roomsRouter.delete("/:roomId/moderators/:userId", requireRole(["admin"]), async (req: Request, res: Response) => {
