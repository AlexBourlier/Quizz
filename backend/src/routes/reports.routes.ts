...
reportsRouter.post("/", async (req: Request, res: Response) => {
  ...
  if (reporterId === reportedId) return res.status(400).json({ message: "Impossible de se signaler soi-même" });

  const target = await prisma.user.findUnique({
    where: { id: reportedId },
    select: { role: { select: { name: true } } },
  });
  if (target?.role.name === "admin") return res.status(403).json({ message: "Impossible de signaler un administrateur" });
